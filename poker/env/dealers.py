...
class SimpleDealer:
    ...
    def game__(self, n):
        ...
        while not end:
            ...
            self.log_step__(n, table_state, action["action"])
            end, table_state = self.table.step(action["action"])
            env_state["now"] = table_state
            env_state["active_positions"] = table_state["active_positions"]
            step += 1
        reward = self.table.get_reward()
        losses = self.brain.save_loss(actions, reward)
    ...
    def checkpoint__(self, n):
        file = self.checkpoint_name + "/" + str(n)
    ...
    def log_step__(self, n, table_state, action):
        ...
        elif action == self.table.bins + 2:
            logging.warning('\tPlayer goes all-in')
        else: 
            logging.warning('\tPlayer bets ' + str(table_state["bet"]))
        logging.warning(' ')

    def log_game__(self, n, rewards, losses):
        rewards = ", ".join(list(map(str, rewards)))
    ...
    def decode_cards__(self, cards):
        ...
        for card in cards:
            if card != -1:
                ...
                prepare_cards.append(rank + " of " + suits[suit]) 
